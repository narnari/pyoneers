...
import time # 돈이 없다는 텍스트 출력 시간 확인 시 필요
...
class State:
    ...
    show_no_land_text = False # 땅 구매 비용 없음 텍스트 출력할지 여부
    show_success_land_text = False # 땅 구매 완료 텍스트 출력할지 여부
    show_no_land_time = 0 # 땅 비용 없음 텍스트 출력할 시간
    show_success_land_time = 0 # 땅 비용 있음 텍스트 출력할 시간
    show_no_tree_text = False # 나무 구매 비용 없음 텍스트 출력할지 여부
    show_success_tree_text = False # 나무 구매 완료 텍스트 출력할지 여부
    show_no_tree_time = 0 # 나무 비용 없음 텍스트 출력할 시간
    show_success_tree_time = 0 # 나무 비용 있음 텍스트 출력할 시간
...
def handle_mouse_click(pos, buttons):
    # 매뉴얼 화면이 열려있으면 뒤로가기 버튼만 클릭 가능
    for btn in buttons:
        if State.is_manual_open and btn.action != handle_back_button:
            continue
        # 버튼의 위치와 마우스 클릭 위치가 겹치는지 확인. 겹치면 버튼 클릭 처리
...
def draw_game(screen):
    ...
    # 아니면 매뉴얼 외 다른 UI 그리기
        ...
        if State.show_no_land_text:
            ...
            if elapsed < 0.75:
                land_editor.draw_no_land_text(screen)
            else:
                State.show_no_land_text = False
        if State.show_success_land_text:
            elapsed = time.time() - State.show_success_land_time
            if elapsed < 0.75:
                land_editor.draw_success_land_text(screen)
            else :
                State.show_success_land_text = False
        
        # 나무 구매 여부 출력
        if State.show_no_tree_text:
            elapsed = time.time() - State.show_no_tree_time
            if elapsed < 0.75:
                tree_editor.draw_no_tree_text(screen)
            else:
                State.show_no_tree_text = False
        if State.show_success_tree_text:
            elapsed = time.time() - State.show_success_tree_time
            if elapsed < 0.75:
                tree_editor.draw_success_tree_text(screen)
            else :
                State.show_success_tree_text = False
        ...
        resource_manager.draw_resources(screen) # 재화 보유량 및 생산량량 출력
...
def run_game(screen):
    ...
    while State.running:
        ...
        resource_manager.update_tree_time(tilemap_drawer.tile_objects) # 업데이트까지 필요한 시간 감소
